...
use crate::matrix;
use crate::metrics;
use crate::models::{ApiKey, JobContext, JobRegistry, JobRequest, JobStatus};
        ...
        if let Some(auth_header) = parts.headers.get(header::AUTHORIZATION) {
            if let Ok(auth_str) = auth_header.to_str() {
                if let Some(token) = auth_str.strip_prefix("Bearer ") {
                    ...
                    if let Ok(Some(api_key)) = q_builder
...
async fn handle_healthz(
    ...
    if db_status.is_ok() && redis_status.is_ok() && docker_status.is_ok() {
        ...
        error!(
            "Health check failed: DB={:?}, Redis={:?}, Docker={:?}",
            db_err, redis_err, docker_err
        );
...
async fn handle_dispatch(
    ...
    // RBAC Check
    if auth.role != "admin"
        && !auth.permissions.contains(&"dispatch".to_string())
        && !auth.permissions.contains(&"*".to_string())
    {
        return (
    ...
    if expanded_jobs.len() > 1 && idempotency_key.is_some() {
        warn!(
            "Idempotency key ignored for matrix job request. It is only supported for single jobs."
        );
    }

    // Pre-generate ID for single job case to use in lock
    let single_job_id = if expanded_jobs.len() == 1 {
            ...
            Ok(false) => {
                ...
                match state.queue.get_idempotency_key(key).await {
                    Ok(Some(existing_job_id)) => {
                        info!(
                            "Idempotency hit for key: {}. Returning existing job_id: {}",
                            key, existing_job_id
                        );
                        return (
                            StatusCode::OK,
    ...
    for job_req in expanded_jobs {
        ...
        let job_id = if expanded_len == 1 {
            single_job_id.clone().unwrap()
        } else {
        ...
        // 1. Persist to DB
        let mut q_builder = sqlx::query_builder::QueryBuilder::new(
            "INSERT INTO jobs (id, status, image, commands, callback_url, user_id, enable_network, run_at) VALUES (",
        );
        ...
        match state.queue.enqueue(context).await {
            Ok(_) => {
                ...
                let _ = q_update.build().execute(&state.db).await;
    ...
    if created_job_ids.is_empty() && !errors.is_empty() {
        ...
        if let Some(key) = idempotency_key {
            if errors.len() == 1 {
                // Only if we tried 1 job and failed
...
async fn handle_cancel_job(
    ...
    // RBAC Check
    if auth.role != "admin"
        && !auth.permissions.contains(&"cancel".to_string())
        && !auth.permissions.contains(&"*".to_string())
    {
        return (
            StatusCode::FORBIDDEN,
        ...
        Ok(Some(row)) => {
            ...
            if log_content.starts_with("s3://") {
                if let (Some(s3), Some(bucket)) = (&state.s3_client, &state.s3_bucket) {
                    ...
                    return (
                        StatusCode::INTERNAL_SERVER_ERROR,
                        Json(serde_json::json!({ "error": "S3 configuration missing" })),
                    )
                        .into_response();
                }
            }
            ...
            (
                StatusCode::OK,
                Json(serde_json::json!({ "logs": log_content })),
            )
                .into_response()
        }
        Ok(None) => (
...
async fn handle_stream_logs(
    ...
    info!(
        "Client connected to stream logs for job: {} (Resume: {:?})",
        job_id, query.last_id
    );
...
async fn handle_list_jobs(
    ...
    let mut q_builder = sqlx::query_builder::QueryBuilder::<BuildDb>::new(
        "SELECT id, status, exit_code, image, created_at, user_id, artifact_url FROM jobs",
    );
...
async fn handle_workflow_dispatch(
    ...
    match workflow::parse_workflow(&body) {
        Ok(graph) => {
            ...
            let mut job_uuid_map: std::collections::HashMap<String, String> =
            ...
            for (key, job_req) in &graph.jobs {
                ...
                let mut q_builder = sqlx::query_builder::QueryBuilder::new(
                    "INSERT INTO jobs (id, status, image, commands, env, services, on_success, callback_url, user_id, enable_network, run_at, workflow_id, dependencies_met) VALUES (",
                );
                q_builder.push_bind(&job_id);
                q_builder.push(", 'PENDING', ");
                q_builder.push_bind(&job_req.image);
            ...
            for (parent_key, children_keys) in &graph.dependencies {
                ...
                for child_key in children_keys {
                    ...
                    let mut q_dep = sqlx::query_builder::QueryBuilder::new(
                    ...
                    if let Err(e) = q_dep.build().execute(&state.db).await {
                        ...
                        return (
                            StatusCode::INTERNAL_SERVER_ERROR,
                            Json(serde_json::json!({"error": "Failed to map dependencies"})),
                        );
                    }
                }
            }
        ...
        Err(e) => (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"error": "Invalid workflow YAML", "details": e.to_string()})),
        ),
    }
}
