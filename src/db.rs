...
pub async fn init_pool() -> Result<DbPool, Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "postgres")]
    {
        ...
        let db_url =
            env::var("DATABASE_URL").expect("DATABASE_URL must be set for PostgreSQL feature");
        ...
        let max_conns = env::var("ZORP_DB_MAX_CONNS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(50);
        let min_conns = env::var("ZORP_DB_MIN_CONNS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(10);
        let timeout = env::var("ZORP_DB_TIMEOUT")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(30);

        info!(
            "⚙️  DB Config (PG): Max={}, Min={}, Timeout={}s",
            max_conns, min_conns, timeout
        );
    ...
    #[cfg(all(feature = "sqlite", not(feature = "postgres")))]
    {
        ...
        warn!(
            "⚠️  WARNING: Running in SQLite mode. This is NOT recommended for production CI/CD workloads."
        );
        ...
        let db_url = match env::var("DATABASE_URL") {
            Ok(url) => {
                if url.starts_with("postgres://") || url.contains("sslmode=") {
                    warn!(
                        "⚠️  Detected PostgreSQL config in DATABASE_URL but running in SQLite mode."
                    );
