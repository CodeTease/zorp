...
pub trait JobQueue: Send + Sync {
    ...
    async fn monitor_stranded_jobs(
        &self,
    ...
    async fn enqueue_upload_dlq(
        &self,
        task: UploadTask,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn dequeue_upload(
        &self,
    ) -> Result<Option<UploadTask>, Box<dyn std::error::Error + Send + Sync>>;
    ...
    async fn enqueue_webhook_retry(
        &self,
        payload: String,
        delay_seconds: i64,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn dequeue_webhook(
        &self,
    ) -> Result<Option<String>, Box<dyn std::error::Error + Send + Sync>>;
    async fn monitor_webhook_retries(
        &self,
    ) -> Result<usize, Box<dyn std::error::Error + Send + Sync>>;

    // Delayed Jobs
    async fn monitor_delayed_jobs(&self)
    -> Result<usize, Box<dyn std::error::Error + Send + Sync>>;
    ...
    async fn set_idempotency_key(
        &self,
        key: &str,
        value: &str,
        ttl_seconds: u64,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn set_idempotency_key_nx(
        &self,
        key: &str,
        value: &str,
        ttl_seconds: u64,
...
impl RedisQueue {
    pub fn new(url: &str) -> Self {
        ...
        if let Ok(mut conn) = queue.client.get_connection() {
            ...
            eprintln!(
                "Failed to connect to Redis synchronously for Group Init. Workers may fail if groups do not exist."
            );
        }
    ...
    async fn process_stream_entry(
        ...
        warn!(
            "Stream entry {} in {} missing payload field or invalid format.",
            id, stream_name
        );
        Ok(None)
    }
...
impl JobQueue for RedisQueue {
    async fn enqueue(
        ...
        if let Some(run_at) = job.run_at {
            ...
            if run_at > now {
                ...
                let _: () = conn
                    .zadd(&self.delayed_queue_zset, payload, score)
                    .await
                    .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
                return Ok(());
            }
        ...
        if let Some(response) = result {
            for stream_key in response.keys {
                ...
                if let Some(entry) = stream_key.ids.first() {
                    ...
                    return self
                        .process_stream_entry(&stream_name, id, map, &mut conn)
                        .await;
                }
            }
        }
    ...
    async fn acknowledge(
        ...
        if let (Some(stream_id), Some(stream_name)) = (&job.stream_id, &job.stream_name) {
            ...
            if ack_count == 0 {
                warn!(
                    "Could not acknowledge job {} from stream. It might have been already ACKed.",
                    job.id
                );
            } else {
            ...
            warn!(
                "Job {} has no stream info. Cannot acknowledge in Redis Streams.",
                job.id
            );
        }
                            ...
                            match serde_json::from_str::<JobContext>(&payload_str) {
                                Ok(mut job) => {
                                    ...
                                    if job.retry_count > 3 {
                                        warn!(
                                            "Job {} (Stream ID {}) exceeded max retries. Moving to DLQ.",
                                            job.id, id
                                        );
                                        let _: () = conn
                                            .lpush(&dlq_name, new_payload)
                                            .await
                                            .map_err(|e| {
                                                Box::new(e)
                                ...
                                Err(e) => {
                                    warn!(
                                        "Poison Pill found! Failed to parse stranded job (Stream ID {}): {}. Moving to DLQ.",
                                        id, e
                                    );
                                    // Move raw payload to DLQ
                                    let _: () =
                                        conn.lpush(&dlq_name, &payload_str).await.map_err(|e| {
                                            Box::new(e) as Box<dyn std::error::Error + Send + Sync>
                                        })?;
    ...
    async fn monitor_delayed_jobs(
        ...
        let items: Vec<String> = conn
            .zrangebyscore(&self.delayed_queue_zset, "-inf", now)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
        ...
        for payload in items {
            ...
            let job: JobContext = match serde_json::from_str(&payload) {
                ...
                Err(e) => {
                    ...
                    let _: () = conn
                        .zrem(&self.delayed_queue_zset, &payload)
                        .await
                        .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
                    continue;
                }
    ...
    async fn update_heartbeat(
        ...
        let ref_val: Option<String> = conn
            .get(&map_key)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn enqueue_upload(
        ...
        let _: () = conn
            .lpush(&self.upload_queue_name, payload)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn dequeue_upload(
        ...
        let result: Option<(String, String)> = conn
            .brpop(&self.upload_queue_name, 1.0)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn enqueue_webhook(
        ...
        let _: () = conn
            .lpush(&self.webhook_queue_name, payload)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
        Ok(())
    }
    ...
    async fn enqueue_webhook_retry(
        ...
        let _: () = conn
            .zadd(&self.webhook_retry_zset, payload, score)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
        Ok(())
    }
    ...
    async fn dequeue_webhook(
        ...
        let result: Option<(String, String)> = conn
            .brpop(&self.webhook_queue_name, 1.0)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn monitor_webhook_retries(
        ...
        // 1. Get ready items
        let items: Vec<String> = conn
            .zrangebyscore(&self.webhook_retry_zset, "-inf", now)
            .await
            .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
        ...
        for item in items {
            ...
            let _: () = conn
                .lpush(&self.webhook_queue_name, &item)
                .await
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;

            // 3. Remove from ZSET
            let _: () = conn
                .zrem(&self.webhook_retry_zset, &item)
                .await
                .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
            count += 1;
        }
