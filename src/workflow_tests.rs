...
"#;
        ...
        let children = graph
            .dependencies
            .get("build")
            .expect("build should have dependencies");
...
"#;
        ...
        let children = graph
            .dependencies
            .get("build")
            .expect("build should have children");
        assert_eq!(children.len(), 4);

        let images: Vec<String> = children
            .iter()
        ...
        let key = children
            .iter()
            .find(|k| graph.jobs.get(*k).unwrap().image == "test:1.0-linux")
            .unwrap();
